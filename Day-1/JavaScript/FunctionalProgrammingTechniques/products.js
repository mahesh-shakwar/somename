...
function groupBy(list, selector) {
    ...
    if (typeof selector === "string") selectorFn = function (p) {
        return p[selector];
    }
    for (var i = 0; i < list.length; i++) {
        var key = selectorFn(list[i]);
...
var productsWithCategoryName = join(products, categories, function (p) {
    return p.category;
}, function (c) {
    return c.id
}, function (p, c) {
